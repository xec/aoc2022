...
let monkeys = input.split("\n\n").map((mstr, id) => {
  ...
  let items = /Starting items: (.*)/.exec(lines[1])[1].split(", ").map(BigInt);
  let [_, operation, param] = /Operation: new = old (.) (.+)/.exec(lines[2]);
  let divisibleBy = BigInt(/Test: divisible by (\d+)/.exec(lines[3])[1]);
...
function inspectAndThrowItem(worryLevel, monkey) {
  ...
  switch (monkey.operation) {
    case "+":
      if (monkey.param === "old") {
        ...
        worryLevel = worryLevel + BigInt(monkey.param);
      }
      break;
    case "*":
      if (monkey.param === "old") {
  ...
  // 3. check divisibleBy
  let isDivisible = worryLevel % monkey.divisibleBy === 0n;
